...
import {
  Box, Typography, Button, Chip, Alert, LinearProgress,
  Card, CardContent, alpha,
} from '@mui/material';
import {
  CloudUpload, PlayArrow, CheckCircle, ErrorOutline,
...
interface BacktestPanelProps {
  gaSessionId: string | number;
  ohlcvSessionId: string | number;
}
...
const BacktestPanel: React.FC<BacktestPanelProps> = ({ gaSessionId, ohlcvSessionId }) => {
  const {
    strategies, selectedStrategy, selectedParameterRow, currentBacktest,
    isRunning, error, setSelectedStrategy, uploadStrategy, runBacktest, clearError,
  } = useBacktestStore();

  const onDrop = useCallback(
    (acceptedFiles: File[]) => { if (acceptedFiles[0]) uploadStrategy(acceptedFiles[0]); },
    [uploadStrategy]
  );
  ...
  const stepIdx = isRunning ? Math.floor(Date.now() / 1500) % STEPS.length : -1;
        ...
        <CardContent sx={{ p: { xs: '14px !important', sm: '20px !important' } }}>
          ...
          <Box
            ...
            sx={{
              ...
              borderRadius: 2, p: { xs: 1.5, sm: 2 }, textAlign: 'center',
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
                {strategies.map((s: Strategy) => (
                  ...
                  >
                    ...
                    <Typography sx={{
                      fontSize: '0.8rem', color: selectedStrategy?.id === s.id ? '#F1F5F9' : '#94A3B8',
      ...
      <AnimatePresence>
        {error && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <Alert severity="error" icon={<ErrorOutline />} onClose={clearError}>{error}</Alert>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isRunning && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <Card>
              <CardContent>
                <Typography variant="subtitle2" sx={{ mb: 2, color: '#F1F5F9' }}>Executing Backtest...</Typography>
      ...
      <Button
        ...
        onClick={() => {
          if (selectedParameterRow)
            runBacktest(selectedParameterRow.id, ohlcvSessionId);
        }}
        sx={{ py: 1.5, fontSize: { xs: '0.875rem', sm: '0.95rem' } }}
      >
